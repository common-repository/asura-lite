				...
				<div class="md:col-span-1">
					<div class="px-4 sm:px-0">
						<h3 class="text-lg font-medium leading-6 text-gray-900">
							Generate license using Generator #{{ initialGenerator.id }}
						</h3>
				...
				<div class="mt-5 md:mt-0 md:col-span-2">
					<div class="shadow sm:rounded-md">
						...
						<div class="px-4 py-5 bg-white sm:p-6">
							<div class="grid grid-cols-6 gap-6">
								...
								<div class="col-span-6 sm:col-span-2">
									<label for="amount" class="block text-sm font-medium leading-5 text-gray-700">Amount <span class="text-red-800">*</span></label>
									<div class="mt-1 relative rounded-md shadow-sm">
										...
										<input type="number" v-model="form.amount" id="amount" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm sm:leading-5 border-gray-300 rounded-md placeholder-gray-500 placeholder-opacity-50" placeholder="1">
									</div>
								</div>
						...
						<div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
							...
							<button @click="doGenerate" type="button" class="px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 sm:order-1 sm:ml-3">
								Generate
							</button>
						</div>
					</div>
	...
	methods: {
		...
		doGenerate() {
			if (!this.$parent.busy) {
                ...
                axios.post(route('admin.api.generators.generate', {generator: this.initialGenerator.id}), {
                ...
                .then(response => {
                    Vue.toasted.show(`Bulk generate license successfully. Total: ${response.data.data.length}`, {
                        type : 'success',
                        iconPack: 'callback',
                        icon: (el) => {
                ...
                .catch(error => {
                    Vue.toasted.show('Failed to bulk generate license', {
                        type : 'error',
                        iconPack: 'callback',
                        icon: (el) => {
                            el.innerHTML = '<svg viewBox="0 0 24 24" height="18" width="18"><g transform="matrix(1,0,0,1,0,0)"><path d="M11.983,0C8.777,0.052,5.72,1.365,3.473,3.653C1.202,5.914-0.052,9.002,0,12.207C-0.008,18.712,5.26,23.992,11.765,24 c0.012,0,0.023,0,0.035,0h0.214c6.678-0.069,12.04-5.531,11.986-12.209l0,0c0.015-6.498-5.24-11.778-11.738-11.794 C12.169-0.003,12.076-0.002,11.983,0z M10.5,16.542c-0.03-0.815,0.606-1.499,1.421-1.529c0.009,0,0.019-0.001,0.028-0.001h0.027 c0.82,0.002,1.492,0.651,1.523,1.47c0.03,0.814-0.605,1.499-1.419,1.529c-0.01,0-0.02,0.001-0.03,0.001h-0.027 C11.203,18.009,10.532,17.361,10.5,16.542z M11,12.5v-6c0-0.552,0.448-1,1-1s1,0.448,1,1v6c0,0.552-0.448,1-1,1S11,13.052,11,12.5z" stroke="none" fill="currentColor" stroke-width="0" stroke-linecap="round" stroke-linejoin="round"></path></g></svg>';
