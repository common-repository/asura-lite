            ...
            <div class="space-y-1 grid grid-cols-2">
                <div class="align-middle">
                    ...
                    <p class="max-w-2xl text-sm text-gray-500">
                        Asura integration with Easy Digital Downloads.
                    </p>
                </div>
                ...
                <dl class="divide-y divide-gray-200">
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:pt-5">
                        ...
                        <dd class="flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <span class="ml-4 flex-shrink-0 flex items-start space-x-4">
                                <select v-model="settings.payment_status.abandoned" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:pt-5">
                        ...
                        <dd class="flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <span class="ml-4 flex-shrink-0 flex items-start space-x-4">
                                <select v-model="settings.payment_status.preapproved" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                                    <option v-for="payment_action in form.payment_actions" :key="payment_action.value" class="capitalize" :value="payment_action.value"> {{ payment_action.value }} </option>
                                </select>
                            </span>
                        </dd>
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:pt-5">
                        ...
                        <dd class="flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <span class="ml-4 flex-shrink-0 flex items-start space-x-4">
                                <select v-model="settings.payment_status.revoked" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                                    <option v-for="payment_action in form.payment_actions" :key="payment_action.value" class="capitalize" :value="payment_action.value"> {{ payment_action.value }} </option>
                                </select>
                            </span>
                        </dd>
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:pt-5">
                        ...
                        <dd class="flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <span class="ml-4 flex-shrink-0 flex items-start space-x-4">
                                <select v-model="settings.payment_status.payment_deleted" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                                    <option v-for="payment_action in form.payment_actions" :key="payment_action.value" class="capitalize" :value="payment_action.value"> {{ payment_action.value }} </option>
                                </select>
                            </span>
                        </dd>
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:pt-5">
                        ...
                        <dd class="flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <span class="ml-4 flex-shrink-0 flex items-start space-x-4">
                                <select v-model="settings.payment_status.item_removed" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                                    <option v-for="payment_action in form.payment_actions" :key="payment_action.value" class="capitalize" :value="payment_action.value"> {{ payment_action.value }} </option>
                                </select>
                            </span>
                        </dd>
    ...
    methods: {
        loadSetting() {
            if (!this.busy) {
                ...
                axios.get(route('admin.settings.section.vendor.easydigitaldownloads.index'), {})
                .then(response => {
                    response.data;
                    this.settings = response.data;
                })
                .catch(error => {
                    Vue.toasted.show('Failed to load Settings [EasyDigitalDownloads]', {
                        type : 'error',
                        iconPack: 'callback',
                        icon: (el) => {
        ...
        doSave() {
            if (!this.busy) {
                ...
                axios.put(route('admin.settings.section.vendor.easydigitaldownloads.update'), {
                    settings: this.settings,
                })
                .then(response => {
                    Vue.toasted.show('Settings [EasyDigitalDownloads] saved successfully.', {
                        type : 'success',
                        iconPack: 'callback',
                        icon: (el) => {
                ...
                .catch(error => {
                    Vue.toasted.show('Failed to save Settings [EasyDigitalDownloads]', {
                        type : 'error',
                        iconPack: 'callback',
                        icon: (el) => {
                            el.innerHTML = '<svg viewBox="0 0 24 24" height="18" width="18"><g transform="matrix(1,0,0,1,0,0)"><path d="M11.983,0C8.777,0.052,5.72,1.365,3.473,3.653C1.202,5.914-0.052,9.002,0,12.207C-0.008,18.712,5.26,23.992,11.765,24 c0.012,0,0.023,0,0.035,0h0.214c6.678-0.069,12.04-5.531,11.986-12.209l0,0c0.015-6.498-5.24-11.778-11.738-11.794 C12.169-0.003,12.076-0.002,11.983,0z M10.5,16.542c-0.03-0.815,0.606-1.499,1.421-1.529c0.009,0,0.019-0.001,0.028-0.001h0.027 c0.82,0.002,1.492,0.651,1.523,1.47c0.03,0.814-0.605,1.499-1.419,1.529c-0.01,0-0.02,0.001-0.03,0.001h-0.027 C11.203,18.009,10.532,17.361,10.5,16.542z M11,12.5v-6c0-0.552,0.448-1,1-1s1,0.448,1,1v6c0,0.552-0.448,1-1,1S11,13.052,11,12.5z" stroke="none" fill="currentColor" stroke-width="0" stroke-linecap="round" stroke-linejoin="round"></path></g></svg>';
